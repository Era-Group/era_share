...
def _patch_llm_api_service():
    ...
    def _request(
        ...
        # LLM responses (esp. with tool-calling / large context) regularly need
        # more than the previous hard-coded 30s. Default to 120s, tunable via the
        # `ai.custom_llm_timeout` system parameter (same idiom as retry_count).
        if timeout is None:
            try:
                timeout = float(_custom_llm_param(
                    self.env, "ai.custom_llm_timeout", "120"))
