...
def _enabled():
    return _icp('era_seo_suite.llms_enabled', 'True') in _TRUE
...
class EraGeoLlms(http.Controller):
    ...
    def _build(self, full=False):
        ...
        max_items = int(_icp('era_seo_suite.llms_max_items', '100') or 100)

        name = website.name or request.env.company.name or 'Website'
        summary = _icp('era_seo_suite.site_summary') or request.env.company.name or ''
        ...
        if _icp('era_seo_suite.llms_include_blog', 'True') in _TRUE:
