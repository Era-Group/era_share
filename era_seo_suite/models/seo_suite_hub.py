...
class EraSeoSuiteHub(models.Model):
    ...
    # Retry count for the finalize commit when it loses a serialization race
    # (40001) against a concurrent update of the SAME post row — e.g. a
    # page-view stored recompute (era_word_count / is_seo_optimized) firing
    # while finalize writes the cover. The rest of the module retries 40001
    # rather than abandoning the work; the finalize path must too, else the
    # cover image is silently rolled back and lost (observed on post 16).
    _FINALIZE_COMMIT_RETRIES = 3
    ...
    def _finalize_one_article(self, post):
        ...
        then clear ``era_ai_needs_finalize``. Ordered so it is re-fire-safe:
          ...
          1. generate the cover bytes ONCE (no DB write — so a commit retry
             never re-calls the image provider);
          2. attach the image + clear the flag + COMMIT, RETRYING on a
             serialization conflict (40001): a concurrent update of the post
             row otherwise rolls the whole attach back and the cover is lost;
          3. send the manager email exactly once (best-effort), only after the
             flag is durably cleared.

        Step 2 before step 3 trades a (rare) missed email on a mid-send kill
        for a guarantee of no DUPLICATE emails. On 40001 we roll the WHOLE
        transaction back (which also undoes the just-created attachment, so no
        orphan) and retry, reusing the pre-generated bytes.
        """
        import time as _time
        post = post.sudo()
        ...
        # 1. Generate the cover bytes up front (no DB write) so a commit retry
        #    below never re-calls the image provider. Best-effort; 3 tries for a
        #    flaky provider; never raises out of here.
        image_bytes = None
        for gen_try in range(1, 4):
            try:
                ...
                _logger.exception(
                    'finalize: image generation attempt %d raised for post#%s',
                    gen_try, post.id)
                image_bytes = None
            if image_bytes:
                break
        if not image_bytes:
            ...
            _logger.info(
                'finalize: no image for post#%s (image account: %s) — '
                'finalizing without a cover.',
                post.id, acc.name if acc else 'none set')

        # 2. Attach + clear flag + commit, retrying on a serialization conflict.
        cleared = False
        for attempt in range(1, self._FINALIZE_COMMIT_RETRIES + 1):
            try:
                if image_bytes:
                    self._attach_article_image(post, image_bytes, article_like)
                post.era_ai_needs_finalize = False
                self.env.cr.commit()
                cleared = True
                break
            except psycopg2.errors.SerializationFailure:
                # Concurrent update of this post row. Roll the WHOLE txn back
                # (also undoing the just-created attachment, so no orphan) and
                # retry with the same pre-generated bytes.
                self.env.cr.rollback()
                if attempt < self._FINALIZE_COMMIT_RETRIES:
                    _logger.info(
                        'finalize: serialization conflict on post#%s '
                        '(attempt %d) — retrying', post.id, attempt)
                    _time.sleep(0.2 * attempt)
                    continue
                _logger.warning(
                    'finalize: gave up on post#%s after %d serialization '
                    'conflicts — left queued for the next tick', post.id, attempt)
                return
            except Exception:  # noqa: BLE001
                # Non-transient attach failure: finalize WITHOUT a cover so a
                # poison post can't stall the queue forever (matches the prior
                # "no image after retries — post kept without a cover").
                self.env.cr.rollback()
                _logger.exception(
                    'finalize: image attach failed for post#%s — finalizing '
                    'without a cover', post.id)
                try:
                    post.era_ai_needs_finalize = False
                    self.env.cr.commit()
                    cleared = True
        ...
        # 3. Notify once, only after the flag is durably cleared.
        try:
