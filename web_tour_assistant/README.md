...
`web_tour_assistant.match_threshold` in system parameters, 0.5 by default.
...
`web_tour_assistant.build_threshold` (0.6). A one-word menu name agrees
...
`web_tour_assistant.generate_tours` to `False` to switch building off and
...
`web_tour_assistant.ai_account`; with none configured the module behaves
...
```bash
odoo-bin -d <database> -u web_tour_assistant --test-enable \
         --test-tags /web_tour_assistant --stop-after-init
```
