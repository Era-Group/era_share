...
class TourAssistantRequest(models.Model):
    ...
    def _match_threshold(self):
        ...
        raw = self.env["ir.config_parameter"].sudo().get_param(
            "web_tour_assistant.match_threshold"
        )
        try:
            value = float(raw)
    ...
    def _answer_source(self):
        ...
        """
        raw = self.env["ir.config_parameter"].sudo().get_param(
            "web_tour_assistant.answer_source", "agent"
        )
    ...
    def _generation_enabled(self):
        ...
        """
        raw = self.env["ir.config_parameter"].sudo().get_param(
            "web_tour_assistant.generate_tours", "True"
        )
    ...
    def _worker_expected(self):
        ...
        """
        raw = self.env["ir.config_parameter"].sudo().get_param(
            "web_tour_assistant.worker_running", "False"
        )
    ...
    def _check_worker_access(self):
        if not self.env.user.has_group(
            "web_tour_assistant.group_tour_assistant_worker"
        ):
            raise AccessError(
